...
class MultiSourceRagSystem:
    ...
    def __init__(self, persist_dir="./chroma_db", chunk_size=420, chunk_overlap=80, min_chunk_length=120):
        """
        初期化メソッド
        
        Args:
            persist_dir: ベクトルデータベースを保存するフォルダのパス
            chunk_size: 1つのチャンクの最大文字数（デフォルト: 420文字）
            chunk_overlap: チャンク間の重複文字数（デフォルト: 80文字）
            min_chunk_length: 最小チャンク長（これより短いチャンクは除外、デフォルト: 120文字）
        """
        # Chroma DBの保存フォルダ
